...
export class BusinessReviewHandler {
  create = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { appointmentId, content, photoKey } = req.body;
      const userId = req.userId!;

      if (!appointmentId || !content) {
        res.status(400).json({ error: 'appointmentId ve content zorunludur.' });
        return;
      }

      const review = await service.create(userId, appointmentId, content, photoKey);
  ...
  update = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { id } = req.params;
      const { content, photoKey } = req.body;
      const userId = req.userId!;
      ...
      const updated = await service.updateById(id, content, photoKey);
